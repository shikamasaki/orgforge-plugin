...
`ledger append` を別に打つ設計で、実地では片側落ちが3回起きた（#8 の refutation が台帳に0件、
#11 の1回目の reject が台帳に無い、`progress_recorded` が0件）。actor は `--by` で既に
渡っているので、分ける理由が無かった。
...
**機械が読めない** — 実地では検証プローブ4件が実判定として集計され、board が「admit 済みだが
...
subagent の turn が**作業の途中で終わる**ことがある。実地で1晩に3件起きた: `status` は
...
1. **「maker が過半だから質が足りていない」は誤り。** 最長 29.8分は 728行の SQL + テスト34件に
...
いなかった** — 実地で1晩運用した結果、監督の失敗5件のうち4件が道具の側で捕まえられる形だった。
...
| 何が漏れたか | 実測 | 対策 |
|---|---|---|
| rework の発注が台帳に残らない | reject/refuted **28件**に対し `rework_requested` が無い（1件は4回 reject で記録0件） | `org_cycle rework` を専用コマンドにし、`verify` が reject 時に**判定の記録と同じ場所**でそのコマンドを出す |
...
`ORG_GITHUB_REPO` も台帳のパスも渡っていなかった。結果、gate と skeptic が計7回、判定を出した
...
テストは、無いのと同じ**（org が #7 の split() で捕まえたのと同じ形が、道具の側にある）。
