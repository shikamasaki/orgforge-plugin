...
!`python3 "${CLAUDE_PLUGIN_ROOT}/tools/reconcile.py" mandate --subjects "$1" --decision "$2" --precedence "$(python3 -c "import yaml,os; print('>'.join(yaml.safe_load(open(os.environ['CLAUDE_PLUGIN_ROOT']+'/template/constitution.yaml'))['mandate_precedence']['order']))")"`
