...
1. **Plan and receipt the tick** — `scripts/tick_host.py <schedule.yaml> --root <ledger> --now-min N [--night]`. The host adapter runs the pure planner and persists its emitted `tick_planned` receipt, so first-run baselining is one-shot and later misses remain detectable. It tells you which checks are DUE, which are SUSPENDED overnight (honor the night fail-safe), and which were MISSED (a due check with no proof-of-run in the ledger). A miss past threshold escalates — surface it, do not swallow it. Do not invoke the pure planner directly from an operating cadence; that bypasses receipt persistence.
