...
- **Build through the SDLC phase order (docs/11): requirements → design → implement → test → integrate → deploy → operate.** Do not jump straight from an intent to code to a deploy — a phase cannot start until the gate has admitted its predecessor (the ledger enforces `phase_started requires_prior phase_admitted`). This is not bureaucracy; it is what makes two makers, handed the same spec, converge on the same process and contracts.
...
- **Nobody reads your diff (docs/11 §4f).** Human review is retired: the mechanical bar, the gate, and the skeptic are the entire judgment layer, so anything only a careful reader would have caught must be caught by a machine instead. Three habits carry that weight, and they are yours, not the gate's:
  - **Prove every new test goes RED.** Break the thing it tests, watch it fail, restore. A test that passes against broken code does not exist — and writing tests to satisfy a coverage number produces exactly that. Put the red output in the work log alongside the green.
  - **Pass environment dependencies as arguments** — clock, home dir, **platform**, fs root — never read deep inside a function. That is what makes time- and platform-dependent behaviour testable without that platform.
  - **Keep the unread-safe bar green as you go** (docs/11 §4e): bounded function size/complexity, strict typing with `any`/`@ts-ignore` banned, no blanket inline suppressions (put an exception in the config *with a reason*), duplication/dead-code scanning, CI on more than one OS. Run `repro_lint check <repo> --phase <phase>` before submitting; the gate rejects a HOLD.
